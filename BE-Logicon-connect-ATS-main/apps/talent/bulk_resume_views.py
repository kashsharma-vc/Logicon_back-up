...
def _is_valid_role_name(name: str) -> bool:
    """Validate that a string is a realistic job role name and not an email, date, phone, address, or garbage."""
    if not name:
        return False
    s = str(name).strip()
    ...
    if '@' in s or re.search(r'@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}', s):
        return False
    # Reject dates or timestamps (e.g. 2026-07-09, 09/07/2026, 09-Jul-2026)
    if (
        '00:00:00' in s or
    ...
    if re.search(r'^\d+\s+(?:no|hut|room|flat|plot|block|sector|chawl|nagar)\b', s, re.IGNORECASE):
        return False
